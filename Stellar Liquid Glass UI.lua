...
function Library.SendNotification(settings)
    ...
    InnerStroke.Color = Color3.fromRGB(255, 228, 196)
...
function Library:create_loader(settings)
    ...
    ContainerStroke.Color = Color3.fromRGB(255, 228, 196)
    ...
    task.spawn(function()
        while ContainerStroke and ContainerStroke.Parent do
            TweenService:Create(ContainerStroke, TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.35}):Play()
            task.wait(1.2)
            if not (ContainerStroke and ContainerStroke.Parent) then break end
            TweenService:Create(ContainerStroke, TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.75}):Play()
            task.wait(1.2)
        end
    end)
    ...
    BarFill.BackgroundColor3 = Color3.fromRGB(255, 228, 196)
...
-- Gives any GuiObject a translucent "glass" look — semi-transparent fill,
-- a soft light-catching gradient, and a faint edge. This does NOT blur
-- whatever's behind it (that's what AcrylicBlur is for, already applied
-- to the main window) — it's the lightweight technique used for individual
-- buttons/cards/toggles where real per-element blur would be too expensive.
--
-- settings = {
--     color = Color3,               -- tint of the glass, default white
--     transparency = 0.82,          -- 0 = opaque, 1 = fully invisible fill
--     corner_radius = UDim.new(1,0) -- default fully rounded (pill/circle)
--     stroke_color = Color3,        -- edge highlight color, default white
--     stroke_transparency = 0.65,
--     stroke_thickness = 1,
--     shine = true,                 -- set false to skip the top-light gradient
-- }
function Library:make_glass(object, settings)
    settings = settings or {}

    if not object or not object:IsA('GuiObject') then
        warn('[Library] make_glass expects a GuiObject (Frame, ImageButton, etc.)')
        return nil
    end

    object.BackgroundColor3 = settings.color or Color3.fromRGB(255, 255, 255)
    object.BackgroundTransparency = settings.transparency or 0.82
    object.BorderSizePixel = 0

    if not object:FindFirstChildOfClass('UICorner') then
        local corner = Instance.new('UICorner')
        corner.CornerRadius = settings.corner_radius or UDim.new(1, 0)
        corner.Parent = object
    end

    local stroke = object:FindFirstChild('GlassStroke') or Instance.new('UIStroke')
    stroke.Name = 'GlassStroke'
    stroke.Color = settings.stroke_color or Color3.fromRGB(255, 255, 255)
    stroke.Thickness = settings.stroke_thickness or 1
    stroke.Transparency = settings.stroke_transparency or 0.65
    stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    stroke.Parent = object

    if settings.shine ~= false then
        local shine = object:FindFirstChild('GlassShine') or Instance.new('UIGradient')
...
function Library:create_ui()
    ...
    Container.BackgroundTransparency = 0.35 -- translucent so the real AcrylicBlur behind it actually shows
    ...
    -- Gradient fill for Container — warm dark bronze instead of pure black,
    -- so the real AcrylicBlur behind actually reads through the glass
    local ContainerGradient = Instance.new("UIGradient")
    ContainerGradient.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0.00, Color3.fromRGB(35, 20, 12)),   -- Top warm bronze
        ColorSequenceKeypoint.new(0.60, Color3.fromRGB(20, 11, 6)),    -- Fade start
        ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 6, 4))      -- Bottom, still dark but not crushed black
    }
    ...
    UIStroke.Color = Color3.fromRGB(255, 228, 196)
    ...
    task.spawn(function()
        while UIStroke and UIStroke.Parent do
            TweenService:Create(UIStroke, TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.35}):Play()
            task.wait(1.2)
            if not (UIStroke and UIStroke.Parent) then break end
            TweenService:Create(UIStroke, TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.75}):Play()
            task.wait(1.2)
        end
    end)
...
AnimateGif(Icon, 60, 40, 2, 3, 5, "rbxassetid://74080484918102", 10)
    ...
    Divider.BackgroundColor3 = Color3.fromRGB(255, 228, 196)
