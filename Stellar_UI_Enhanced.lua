...
AnimateGif(Icon, 60, 40, 2, 3, 5, "rbxassetid://74080484918102", 10)
    ...
    function self:create_tab(title: string, icon: string)
        local TabManager = {}
        local LibrarySelf = self -- captured so nested module/keybind closures can check UI visibility without 'self' being shadowed
...
function TabManager:create_image(settings: any)
    ...
    ImageCorner.Parent = Image
        ...
        function TabManager:create_module(settings: any)
            ...
            UsedIndicator.Position = UDim2.new(0, 0, 0, 0)
            UsedIndicator.Size = UDim2.new(0, 3, 1, 0)
            UsedIndicator.BorderSizePixel = 0
            ...
            UsedIndicatorCorner.Parent = UsedIndicator

            local usedIndicatorGlowLoop = nil -- task handle so it can be stopped when the module turns off

            local function stop_used_indicator_glow()
                if usedIndicatorGlowLoop then
                    task.cancel(usedIndicatorGlowLoop)
                    usedIndicatorGlowLoop = nil
                end
            end

            local function start_used_indicator_glow()
                stop_used_indicator_glow()
                usedIndicatorGlowLoop = task.spawn(function()
                    while UsedIndicator and UsedIndicator.Parent do
                        TweenService:Create(UsedIndicator, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {BackgroundTransparency = 0}):Play()
                        task.wait(1)
                        if not (UsedIndicator and UsedIndicator.Parent) then break end
                        TweenService:Create(UsedIndicator, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {BackgroundTransparency = 0.55}):Play()
                        task.wait(1)
                    end
                end)
            end
            ...
            function ModuleManager:change_state(state: boolean)
                ...
                if self._state then
                    ...
                    -- ✅ First activation reveals the used-indicator; every activation resumes the glow pulse
                    start_used_indicator_glow()
                else
                    TweenService:Create(Module, TweenInfo.new(0.5, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                    ...
                    -- ✅ Stop pulsing but leave the bar dimly visible — it marks this module as "has been used"
                    stop_used_indicator_glow()
                    TweenService:Create(UsedIndicator, TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
            ...
            function ModuleManager:connect_keybind()
                ...
                Connections[settings.flag..'_keybind'] = UserInputService.InputBegan:Connect(function(input: InputObject, process: boolean)
                    ...
                    -- ✅ Ignore module keybinds while the UI is minimized
                    if not LibrarySelf._ui_open then
                        return
                    end
...
function ModuleManager:create_button(settings: any)
    ...
    return Button
            ...
            function ModuleManager:create_divider(settings: any)
                ...
                local function make_line_segment(name: string, width: number, xOffset: number)
                    local Divider = Instance.new('Frame')
                    Divider.Size = UDim2.new(0, width, 0, dividerHeight)
                    Divider.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
                    Divider.BorderSizePixel = 0
                    Divider.Name = name
                    Divider.Parent = OuterFrame
                    Divider.ZIndex = 2
                    Divider.Position = UDim2.new(0, xOffset, 0.5, -dividerHeight / 2)
                ...
                if settings and settings.showtopic and settings.title then
                    -- ✅ Custom text divider: measure the label, then draw two line segments
                    -- that stop on either side of it (matches the reference "── Title ──" look)
                    local font_params = Instance.new('GetTextBoundsParams')
                    font_params.Text = settings.title
                    font_params.Font = Font.new('rbxasset://fonts/families/GothamSSm.json', Enum.FontWeight.SemiBold, Enum.FontStyle.Normal)
                    font_params.Size = 11
                    font_params.Width = 10000

                    local font_size = TextService:GetTextBoundsAsync(font_params)
                    local gap = 8 -- space between each line segment and the text
                    local textWidth = font_size.X
                    local segmentWidth = math.max((dividerWidth - textWidth - (gap * 2)) / 2, 4)
                    ...
                    TextLabel.Size = UDim2.new(0, textWidth, 0, 13)
                    TextLabel.Position = UDim2.new(0.5, 0, 0.5, 0)
                    TextLabel.BackgroundTransparency = 1
                    ...
                    if not settings.disableline then
                        make_line_segment('DividerLeft', segmentWidth, 0)
                        make_line_segment('DividerRight', segmentWidth, dividerWidth - segmentWidth)
                    end
                elseif not settings or not settings.disableline then
                    -- Plain full-width fading line (original behavior, no title given)
                    local Divider = Instance.new('Frame')
